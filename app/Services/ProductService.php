...
class ProductService
{
    ...
    public function getAddProductData(object $request, string $addedBy, int|string $shopId): array
    {
        ...
        return [
            ...
            'request_status' => $addedBy == 'admin' ? 1 : (($request['product_type'] === 'physical' && empty($request['location_city_id'])) || getWebConfig(name: 'new_product_approval') == 1 ? 0 : 1),
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        $dataArray = [
            ...
            'discount_type' => $request['discount_type'],
            'attributes' => $request['product_type'] == 'physical' ? json_encode($request['choice_attributes']) : json_encode([]),
            'current_stock' => $request['product_type'] == 'physical' ? abs($stockCount) : $product['current_stock'],
            'minimum_order_qty' => $request['minimum_order_qty'],
            'video_provider' => 'youtube',
            'video_url' => $request['video_url'],
        ...
        if ($updateBy == 'seller' && $product->request_status !== 2 && $request['product_type'] === 'physical' && empty($request['location_city_id'])) {
            $dataArray += [
                'request_status' => 0,
            ];
        }

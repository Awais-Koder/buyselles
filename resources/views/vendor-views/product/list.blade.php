        ...
        <div class="row mt-20">
            <div class="col-md-12">
                <div class="card">
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                                ...
                                <div class="position-relative">
                                    @if (
                                        ...
                                        <div class="position-absolute inset-inline-end-0 top-0 mt-n1 me-n1 btn-circle bg-danger border border-white border-2 z-2"
                                            style="--size: 12px;"></div>
                                    @endif
                                    <button type="button"
                                        @if (
                                            ...
                                            @else
                                                class="btn btn-outline--primary px-4" @endif
                            ...
                            <tbody>
                                @foreach ($products as $key => $product)
                                    <tr>
                                            ...
                                            <a href="{{ route('vendor.products.view', [$product['id']]) }}"
                                                ...
                                                <div>
                                                    <div class="d-flex align-items-center gap-1">
                                                        <div class="media-body title-color mb-1 hover-c1"
                                                            data-toggle="tooltip" title="{{ $product['name'] }}">
                                                            {{ Str::limit($product['name'], 20) }}
                                                        </div>
                                                        @if ($product?->clearanceSale)
                                                            <span class="text-secondary-base fs-12" data-placement="right"
                                                                data-toggle="tooltip"
                                                    ...
                                                    <div class="d-flex gap-1">
                                                        ...
                                                        @if ($product->request_status == 0)
                                                            <label
                                                                class="badge badge-soft-warning m-0 font-weight-normal">{{ translate('pending') }}</label>
                                                        @elseif($product->request_status == 1)
                                                            <label
                                                                class="badge badge-soft-success m-0 font-weight-normal">{{ translate('approved') }}</label>
                                                        @elseif($product->request_status == 2)
                                                            <label
                                                                class="badge badge-soft-danger m-0 font-weight-normal">{{ translate('denied') }}</label>
                                                        @endif
                                                    </div>
                                                </div>
                                            </a>
                                        ...
                                        <td>
                                            <div
                                                class="d-flex justify-content-center mx-auto w-80px gap-3 align-items-center lh-1">
                                                @if ($product['product_type'] === 'physical')
                                                    <span>{{ $product->current_stock }}</span>
                                                    @if ($product->current_stock <= 0)
                                                        <span class="text-danger-dark fs-18" data-toggle="tooltip"
                                                            data-placement="right" title="{{ translate('Out_of_Stock') }}">
                                                ...
                                                @if ($type != 'new-request')
                                                    ...
                                                    @if ($product['product_type'] === 'digital')
                                                        <a class="btn btn-outline-primary btn-sm square-btn"
                                                            title="{{ translate('Upload Codes') }}"
                                                            href="{{ route('vendor.products.digital-code-import.product-import', $product['id']) }}">
                    ...
                    @if (count($products) == 0)
                        @include(
                            'layouts.vendor.partials._empty-state',

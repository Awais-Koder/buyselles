                ...
                <ul class="list-unstyled profile-menu gap-1 mt-3">
                    ...
                    <li class="{{ Request::is('chat/vendor') || Request::is('chat/delivery-man') ? 'active' : '' }}">
                        <a href="{{ route('chat', ['type' => 'vendor']) }}">
                            ...
                            @auth('customer')
                                <?php $customerUnreadChat = \App\Models\Chatting::where(['seen_by_customer' => 0, 'user_id' => auth('customer')->id()])
                                    ->whereRaw('(sent_by_seller = 1 OR sent_by_admin = 1 OR sent_by_delivery_man = 1)')
                                    ->count(); ?>
                                @if ($customerUnreadChat > 0)
                                    <span class="badge badge-sm bg-danger rounded-circle px-1 ml-1"
                                        style="font-size: 10px; min-width: 18px; line-height: 14px;">{{ $customerUnreadChat > 99 ? '99+' : $customerUnreadChat }}</span>
                                @endif
                            @endauth
                        </a>
                    </li>
                    ...
                    <li class="d-lg-none">
                        <a class="d-flex align-items-center" href="javascript:" class="delete-action"
                            data-action="{{ route('account-delete', [$customer_info['id']]) }}"
                            data-text="{{ translate('want_to_delete_this_account') . '?' }}">

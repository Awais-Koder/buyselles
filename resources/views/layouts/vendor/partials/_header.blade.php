@php
    ...
    $sellerCreatedAt = auth('seller')->user()->created_at;
    $notification = Notification::whereBetween('created_at', [$sellerCreatedAt, Carbon::now()])
        ->where('sent_to', 'seller')
        ->whereDoesntHave('notificationSeenBy')
        ->count();
    $notification_data = Notification::whereBetween('created_at', [$sellerCreatedAt, Carbon::now()])
        ->where('sent_to', 'seller')
        ->with('notificationSeenBy')
        ->latest()
        ->get();
    $messageCustomer = Chatting::where(['seen_by_seller' => 0, 'seller_id' => auth('seller')->id()])
        ->whereNotNull(['user_id'])
        ->count();
    $messageDeliveryMan = Chatting::where(['seen_by_seller' => 0, 'seller_id' => auth('seller')->id()])
        ->whereNotNull(['delivery_man_id'])
        ->count();
    $local = session()->has('local') ? session('local') : 'en';
    $lang = \App\Utils\VendorHeaderManager::getLanguageSetting();
    $sellerId = auth('seller')->id();
    $message = \App\Utils\VendorHeaderManager::getUnreadMessageCount($sellerId);
    $order = \App\Utils\VendorHeaderManager::getPendingOrderCount($sellerId);
    $websiteMessageCount = \App\Utils\VendorHeaderManager::getSeenMessageCount($sellerId);
@endphp
    ...
    <header id="header"
        ...
        <div class="navbar-nav-wrap">
            ...
            <div class="navbar-nav-wrap-content-right"
                ...
                <ul class="navbar-nav align-items-center flex-row gap-2 gap-xl-16px">
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a title="{{ translate('website_shop_view') }}"
                                ...
                                href="{{ route('vendor-shop', ['slug' => $shop['slug']]) }}" target="_blank"
                        ...
                        <div class="hs-unfold">
                            <div>
                                <div
                                    ...
                                    <a class="topbar-link d-flex align-items-center" href="javascript:"
                        ...
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle media align-items-center gap-3 navbar-dropdown-account-wrapper dropdown-toggle-left-arrow dropdown-toggle-empty"
                                ...
                                <span id="physical-order-new-badge"
                                    style="display:none; position:absolute; top:2px; right:2px; width:10px; height:10px; background:#dc3545; border-radius:50%; border:2px solid #fff;"
                    ...
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle media align-items-center gap-3 navbar-dropdown-account-wrapper dropdown-toggle-left-arrow dropdown-toggle-empty"
                                ...
                                <span id="vendor-message-count-badge" class="btn-status btn-sm-status btn-status-danger"
                                    style="{{ $message == 0 ? 'display:none;' : '' }}">{{ $message }}</span>
                            </a>
                            <div id="messageDropdown"
                                ...
                                <a class="dropdown-item position-relative"
                                    ...
                                    <span id="vendor-message-customer-count"
                                        class="btn-status btn-sm-status-custom btn-status-danger"
                                        style="{{ $messageCustomer == 0 ? 'display:none;' : '' }}">{{ $messageCustomer }}</span>
                                </a>
                                <div class="dropdown-divider"></div>
                                <a class="dropdown-item position-relative"
                                    ...
                                    <span id="vendor-message-delivery-count"
                                        class="btn-status btn-sm-status-custom btn-status-danger"
                                        style="{{ $messageDeliveryMan == 0 ? 'display:none;' : '' }}">{{ $messageDeliveryMan }}</span>
                                </a>
                            </div>
                        </div>
                    </li>
                    ...
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle"
                                ...
                                <span id="vendor-pending-order-count-badge"
                                    class="btn-status btn-sm-status btn-status-danger"
                                    style="{{ $order == 0 ? 'display:none;' : '' }}">{{ $order }}</span>
                            </a>
                        </div>
                    </li>
                    ...
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a class="p-0 js-hs-unfold-invoker media align-items-center gap-3 navbar-dropdown-account-wrapper dropdown-toggle-left-arrow"
                                href="javascript:"
                                data-hs-unfold-options='{
                                ...
                                {{-- <div class="d-none d-md-block media-body text-right">
                                    ...
                                    <span class="fs-12">{{ Str::limit($shop->name, 20) }}</span>
                                </div> --}}
        ...
        <div id="website_info" class="bg-secondary w-100 d-none">
            <div class="p-3">
                ...
                <div class="bg-white p-1 rounded mt-2">
                    <a class="p-2  title-color" href="{{ route('vendor.messages.index', ['type' => 'customer']) }}"
                        title="{{ translate('message') }}" data-toggle="tooltip"
                        data-custom-class="header-icon-title">
                        <svg width="20" height="21" viewBox="0 0 20 21" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                        ...
                        {{ translate('message') }}
                        @if ($websiteMessageCount != 0)
                            <span>({{ $websiteMessageCount }})</span>
                        @endif
                    </a>
                </div>
                <div class="bg-white p-1 rounded mt-2">
                    <a class="p-2 title-color" href="{{ route('vendor.orders.list', ['pending']) }}"
                        title="{{ translate('Shopping Cart') }}" data-toggle="tooltip"
                        data-custom-class="header-icon-title">
                        <svg width="20" height="21" viewBox="0 0 20 21" fill="none"
                            xmlns="http://www.w3.org/2000/svg">

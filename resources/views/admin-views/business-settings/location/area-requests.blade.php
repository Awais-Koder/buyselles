...
@section('title', translate('location_Setup') . ' - ' . translate('area_requests'))
                ...
                <div class="card">
                    <div class="card-body d-flex flex-column gap-20">
                        <div class="d-flex justify-content-between align-items-center gap-20 flex-wrap">
                            <h3 class="mb-0">
                                {{ translate('vendor_area_requests') }}
                                ...
                                <form action="{{ url()->current() }}" method="GET" class="d-flex gap-2">
                                    ...
                                    <div class="input-group flex-grow-1 max-w-280">
                                        <input type="search" name="searchValue" class="form-control"
                                            placeholder="{{ translate('search_by_area_name') }}" value="{{ $searchValue }}">
                            ...
                            <table class="table table-hover table-borderless align-middle">
                                <thead class="text-capitalize">
                                    <tr>
                                        ...
                                        <th>{{ translate('requested_area') }}</th>
                                ...
                                <tbody>
                                    @foreach ($areaRequests as $key => $request)
                                        <tr>
                                            <td>{{ $areaRequests->firstItem() + $key }}</td>
                                            <td>
                                                @if ($request->seller)
                                                    {{ $request->seller->f_name }} {{ $request->seller->l_name }}
                                                @else
                                                    <span class="text-muted">{{ translate('guest_or_deleted_vendor') }}</span>
                                                @endif
                                            </td>
                                            <td>{{ $request->city?->country?->name ?? '-' }}</td>
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-2">
                                                    @if ($request->status === 'pending')
                                                        <form action="{{ route('admin.business-settings.location.approve-area-request', $request->id) }}"
                                                        ...
                                                        <button class="btn btn-sm btn-outline-danger"
                                                            ...
                                                            data-bs-target="#rejectAreaModal-{{ $request->id }}">
                                                            <i class="fi fi-sr-cross"></i> {{ translate('reject') }}
                                                        </button>
                                                    @elseif ($request->status === 'approved')
                                                        <span class="text-success">
                                                            <i class="fi fi-sr-check"></i>
                                                            {{ $request->approvedArea?->name ?? translate('area_created') }}
                                                        </span>
                                                    @else
                                        ...
                                        {{-- Reject Modal --}}
                                        @if ($request->status === 'pending')
                                            <div class="modal fade" id="rejectAreaModal-{{ $request->id }}" tabindex="-1" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <form action="{{ route('admin.business-settings.location.reject-area-request', $request->id) }}" method="POST">
                                                            @csrf
                                                            <div class="modal-header">
                                                                <h5 class="modal-title">{{ translate('reject_area_request') }}</h5>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                            </div>
                                                            <div class="modal-body">
                                                                <p>
                                                                    {{ translate('vendor') }}: <strong>{{ $request->seller?->f_name }} {{ $request->seller?->l_name }}</strong><br>
                                                                    {{ translate('city') }}: <strong>{{ $request->city?->name }}</strong><br>
                                                                    {{ translate('requested_area') }}: <strong>{{ $request->area_name }}</strong>
                                                                </p>
                                                                <div class="mb-3">
                        ...
                        <div class="table-responsive mt-4">
                            <div class="d-flex justify-content-lg-end">
                                {{ $areaRequests->appends(request()->query())->links() }}
                            </div>
                        </div>

                        @if (count($areaRequests) == 0)
                            @include('layouts.admin.partials._empty-state', [
                                'text' => 'no_area_requests_found',
                                'image' => 'default',
                            ])
                        @endif
                    </div>

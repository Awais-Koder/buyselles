...
@section('content')
                                    ...
                                    @foreach ($subCategories as $sub)
                                        <div class="col-lg-3 col-md-3 col-sm-4 col-6 p-2">
                                            <a href="{{ route('category-products', ['slug' => $sub->slug]) }}"
                                                class="card text-center text-decoration-none border rounded-3 overflow-hidden h-100 sub-cat-card"
    ...
    <span id="products-search-data-backup" data-page="{{ request('page') ?? 1 }}" data-url="{{ url()->current() }}"
        data-brand="{{ $data['brand_id'] ?? '' }}" data-category="{{ $data['category_id'] ?? '' }}"
        data-name="{{ $data['name'] }}" data-offer-type="{{ $data['offer_type'] }}"

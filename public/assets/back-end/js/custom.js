...
$("#toggle-modal-ok-button").on("click", function () {
    ...
    if (toggleIdName === "email-verification") {
        if (
            ...
            toastMagic.info($("#get-email-and-otp-verification-info-message").data("info"));
        }
    }
    if (
                    ...
                    success: function (response) {
                        if (dataFrom == "currency") {
                            if (response.status == 1) {
                                toastMagic.success($("#get-delete-currency-message").data("success"));
                            } else {
                                toastMagic.warning($("#get-delete-currency-message").data("warning"));
                            }
                        } else {
                            toastMagic.success($("#get-deleted-message").data("text"));
                        }
    ...
    function () {
        ...
        if (fromDate !== "" && toDate !== "") {
            if (fromDate > toDate) {
                elementFromDate.val("");
                elementToDate.val("");
                toastMagic.error($("#message-invalid-date-range").data("text"));
            }
        }
    }
);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (
                ...
                if (
                    response?.restockProductCount > 0 &&
                    response?.restockProduct
                ) {

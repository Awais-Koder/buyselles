    ...
    <div class="content container-fluid">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
            ...
            @if (getActiveAIProviderConfigCache())
                <div class="form-control border-0 badge-pill bg-white d-flex justify-content-center align-items-center px-12 py-2 shadow-sm w-auto text-nowrap"
                    id="ai-remaining-count">
                    <span class="d-flex gap-2">
                        <span class="fw-bold" id="count">{{ $aiRemainingCount ?? 0 }}</span>
                        {{ translate('generates_left') }}
                    ...
                    <button type="button" class="btn btn-lg rounded-circle shadow-lg" data-toggle="modal"
                        ...
                        <span class="position-relative z-1 text-white d-flex flex-column gap-1 align-items-center">
                            <img width="16" height="17"
                                src="{{ dynamicAsset(path: 'public/assets//back-end/img/ai/hexa-ai.svg') }}"
                                alt="">
...
@push('script')
    ...
    <script>
        ...
        let imageCount = {{ 15 - count(json_decode($product->images ?? 'null') ?? []) }};
        let thumbnail =
        ...
        $(document).on('change', '.action-upload-color-image', function() {
            ...
            if (input.files && input.files[0]) {
                let maxFileSizeLimit = Number(input?.dataset?.maxSize || ($('#imageUploadMaxSize').data(
                    'max-size') || 20));

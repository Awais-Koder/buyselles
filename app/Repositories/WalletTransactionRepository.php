...
class WalletTransactionRepository implements WalletTransactionRepositoryInterface
{
    ...
    {
        if ($this->businessSetting->where('type', 'wallet_status')->first()->value != 1) {
            return false;
        }
        ...
        try {
            ...
            DB::commit();
            if (in_array($transactionType, ['loyalty_point', 'order_place', 'add_fund_by_admin'])) {

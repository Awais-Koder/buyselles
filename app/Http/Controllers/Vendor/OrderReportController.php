...
class OrderReportController extends Controller
{
    ...
    public function order_report(Request $request)
    {
        ...
        $payment_data = self::getVendorOrderReportPaymentFormattedData(request: $request);
        ...
        return view('vendor-views.report.order-report', compact('orders', 'order_count', 'payment_data', 'chart_data', 'due_amount', 'settled_amount', 'search', 'date_type', 'from', 'to', 'chartDataOrderAmountLabel'));
    }

    public function getVendorOrderReportPaymentFormattedData(object|array $request): array
    {
        ...
        $digitalPayment = self::pie_chart_and_order_count_common_query($request, $digitalPaymentQuery)->sum('init_order_amount');

        $cashPaymentQuery = Order::where(['order_status' => 'delivered'])->whereIn('payment_method', ['cash', 'cash_on_delivery']);
        $cashPayment = self::pie_chart_and_order_count_common_query($request, $cashPaymentQuery)->sum('init_order_amount');

        $walletPaymentQuery = Order::where(['order_status' => 'delivered'])->where(['payment_method' => 'pay_by_wallet']);
        $walletPayment = self::pie_chart_and_order_count_common_query($request, $walletPaymentQuery)->sum('init_order_amount');

        $offlinePaymentQuery = Order::where(['order_status' => 'delivered'])->where(['payment_method' => 'offline_payment']);
        $offlinePayment = self::pie_chart_and_order_count_common_query($request, $offlinePaymentQuery)->sum('init_order_amount');

        $orderEditHistory = self::pie_chart_and_order_count_common_query($request, Order::where(['order_status' => 'delivered'])->with(['orderEditHistory']))->get();
    ...
    {
        return Order::where('order_status', 'delivered')->where(['seller_is' => 'seller', 'seller_id' => auth('seller')->id()])

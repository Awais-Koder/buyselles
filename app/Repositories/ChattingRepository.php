...
class ChattingRepository implements ChattingRepositoryInterface
{
    ...
    {
        return $this->chatting->find($id)->update($data);
    }
    ...
    {
        $dynamicColumn = $data['usersColumn'];
        $dynamicFilterColumn = array_keys($data)[0];

        return $this->chatting
            ->selectRaw("{$dynamicColumn}, COUNT(*) as unread_count")
            ->where($dynamicFilterColumn, $data[$dynamicFilterColumn])
            ->where($data['filteredByColumn'], 0)
            ->groupBy($dynamicColumn)
            ->get()
            ->pluck('unread_count', $dynamicColumn)
            ->toArray();
    }
}

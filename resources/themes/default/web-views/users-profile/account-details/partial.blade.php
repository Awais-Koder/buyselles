...
<div class="d-flex align-items-start justify-content-between flex-sm-nowrap flex-wrap gap-3 mb-2">
    <div>
        <div class="d-flex align-items-center gap-2 text-capitalize">
            <h4 class="text-capitalize mb-0 mobile-fs-14 fs-18 font-bold">{{ translate('order') }} #{{ $order->id }}
            </h4>
            ...
            @elseif(
                $order['order_status'] == 'failed' || $order['order_status'] == 'canceled' || $order['order_status'] == 'returned'
                    ? 'badge-soft-danger'
                    : '')
                <span
    ...
    <div class="d-flex align-items-center gap-2">
        @if ($isEligibleForRefundButtonShow > 0)
            <button class="btn rounded btn-outline-primary py-1 px-3" data-toggle="modal"
                data-target="#refund_request">{{ translate('refund') }}</button>
        @endif
...
@foreach ($order->details as $key => $detail)
    ...
    @if ($product)
        @include('layouts.front-end.partials.modal._review', [
            'id' => $detail->id,
            'order_details' => $detail,
        ])
        @include('layouts.front-end.partials.modal._refund', [
            'id' => $detail->id,
            'order_details' => $detail,
            'order' => $order,
            'product' => $product,

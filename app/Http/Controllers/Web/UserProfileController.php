...
class UserProfileController extends Controller
{
    ...
    public function address_store(Request $request): RedirectResponse
    {
        ...
        $address = [
            ...
            'address_type' => Str::lower($request['address_type'] ?? ''),
    ...
    public function address_update(Request $request): RedirectResponse
    {
        $request->validate([
            'name' => 'required',
            'phone' => 'required|max:20',
    ...
    public function account_order_details(Request $request): View|RedirectResponse
    {
        ...
        if ($order) {
            ...
            return view(VIEW_FILE_NAMES['account_order_details'], [
                ...
                'cashOnDeliveryStatus' => getWebConfig(name: 'cash_on_delivery')['status'],
    ...
    public function account_order_details_delivery_man_info(Request $request)
    {
        ...
        return view(VIEW_FILE_NAMES['delivery_man_info'], compact('delivered_count', 'order', 'paymentGatewayList', 'cashOnDeliveryStatus', 'offlinePaymentMethods', 'offlinePaymentStatus'));
    }
    ...
    public function account_disputes(): View|RedirectResponse
    {
        if (! auth('customer')->check()) {
            return redirect()->route('home');
        }

        $disputes = Dispute::where('buyer_id', auth('customer')->id())
            ->with(['reason', 'order'])
            ->latest()
            ->paginate(10);

        return view(VIEW_FILE_NAMES['account_disputes'], compact('disputes'));
    }

    public function account_dispute_details(int $id): View|RedirectResponse
    {
        if (! auth('customer')->check()) {
            return redirect()->route('home');
        }
        ...
        $dispute = Dispute::where('id', $id)
            ->where('buyer_id', auth('customer')->id())
            ->with([
                'reason',
                'order',
        ...
        if (! $dispute) {
            Toastr::warning(translate('dispute_not_found'));
        ...
        $canEscalate = in_array($dispute->status, [
            DisputeStatus::OPEN,
            DisputeStatus::VENDOR_RESPONSE,
        ]);

        return view(VIEW_FILE_NAMES['account_dispute_details'], compact('dispute', 'isClosed', 'canEscalate'));
    }

...
use App\Http\Requests\Admin\ClearanceSaleSEOSetupRequest;
...
class ClearanceSaleController extends BaseController
{
    ...
    public function getView(Request $request): View
    {
        ...
        $clearanceConfig = $this->stockClearanceSetupRepo->getFirstWhere(params: ['setup_by' => 'vendor', 'user_id' => auth('seller')->id()], relations: ['seo']);
    ...
    public function updateClearanceSeoConfig(ClearanceSaleSEOSetupRequest $request): RedirectResponse
    {
        $clearanceConfig = $this->stockClearanceSetupRepo->getFirstWhere(params: ['setup_by' => 'vendor', 'user_id' => auth('seller')->id()], relations: ['seo']);
        if ($clearanceConfig) {

            $seoMetaData = $this->seoMetaInfoService->getModelSEOData(request: $request, seoMetaInfo: $clearanceConfig?->seo, type: 'App\Models\StockClearanceSetup', modelId: $clearanceConfig->id, action: 'update');
            $this->seoMetaInfoRepo->updateOrInsert(params: ['seoable_type' => 'App\Models\StockClearanceSetup', 'seoable_id' => $clearanceConfig['id']], data: $seoMetaData);

            ToastMagic::success(translate('Meta_updated_successfully'));
        } else {

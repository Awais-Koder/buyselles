...
$('.add-to-cart-from-wishlist').on('click', function (e) {
    ...
    let product_variation_code = $(this).data('product_variation_code');
...
function updateProductDetailsTopSection(formSelector, response) {
    console.log("top section", response);
    $(".add-to-cart-details-form").find(".product-details-chosen-price-section").removeClass("d-none");
    $(".add-to-cart-details-form").find(".product-details-chosen-price-amount").html(response?.price);
    $(".add-to-cart-details-form").find(".product-details-stock-qty").html(response?.quantity);

    if (response?.product_type?.toString() === 'physical') {
        let productRestockRequestButton = $(".add-to-cart-details-form").find(".product-restock-request-button");
        if (response?.quantity <= 0) {
            $(".add-to-cart-details-form").find(".product-restock-request-section").show();
            productRestockRequestButton.removeAttr('disabled');
            $(".add-to-cart-details-form").find(".product-add-and-buy-section").hide().removeClass('d-flex');
        } else {
            $(".add-to-cart-details-form").find(".product-restock-request-section").hide();
            $(".add-to-cart-details-form").find(".product-add-and-buy-section").show().addClass('d-flex');
        }
    ...
    if (response?.product_type?.toString() === 'digital') {
        if (response?.quantity <= 0) {
            $(".add-to-cart-details-form").find(".product-out-of-stock-section").show();
            $(".add-to-cart-details-form").find(".product-add-and-buy-section").hide().removeClass('d-flex');
        } else {
            $(".add-to-cart-details-form").find(".product-out-of-stock-section").hide();
            $(".add-to-cart-details-form").find(".product-add-and-buy-section").show().addClass('d-flex');
        }
    }

    $(".add-to-cart-details-form").find(".product-details-cart-qty").val(response?.in_cart_quantity);
    $(".add-to-cart-details-form").find(".product-details-cart-qty").attr("max", response?.quantity);
    $(".add-to-cart-details-form").find(".product-generated-variation-code").val(response?.variation_code);
    $(".add-to-cart-details-form").find(".product-generated-variation-text").text(response?.variation_code);

    if ($(".add-to-cart-details-form").find(".product-details-cart-qty").attr("min") < parseFloat($(".add-to-cart-details-form").find(".product-details-cart-qty").val())) {
        setTimeout(() => {
            $(".add-to-cart-details-form").find(".btn-number[data-type='minus'][data-field='quantity']").removeAttr("disabled");
        }, 1000);
    } else {
        $(".add-to-cart-details-form").find(".btn-number[data-type='minus'][data-field='quantity']").attr("disabled", true);
    }

    $(".add-to-cart-details-form").find(".discounted-unit-price").html(response?.discounted_unit_price);
    $(".add-to-cart-details-form").find(".product-total-unit-price").html(response?.discount_amount > 0 ? response?.total_unit_price : "");

    let actionAddToCartBtn = $(".add-to-cart-details-form").find(".product-add-to-cart-button");
    ...
    if (response?.discount_amount > 0) {
        if (response?.discount_type === 'flat') {
            $(".add-to-cart-details-form").find(".discounted_badge").html(`${response?.discount}`);
        } else {
            $(".add-to-cart-details-form").find(".discounted_badge").html(`- ${response?.discount}`);
        }
        $(".add-to-cart-details-form").find(".discounted-badge-element").removeClass('d-none');
    } else {
        $(".add-to-cart-details-form").find(".discounted-badge-element").addClass('d-none');
    }
}

function updateProductDetailsBottomSection(formSelector, response) {
    console.log("bottom section", response);
    let productDetailsStickySection = $(".product-details-sticky-section");
    $(".product-details-chosen-price-amount").text(response?.price);
    $(".add-to-cart-sticky-form .product_quantity__qty").val(response?.in_cart_quantity).attr("max", response?.quantity);
    ...
    let actionAddToCartBtn = $(".add-to-cart-sticky-form .product-add-to-cart-button");
    if (response?.in_cart_status === 1) {
        actionAddToCartBtn.html(actionAddToCartBtn.data("update"));
    } else {
        actionAddToCartBtn.html(actionAddToCartBtn.data("add"));
    ...
    if (response?.product_type?.toString() === "physical") {
        let productRestockRequestButton = $(".add-to-cart-sticky-form .product-restock-request-button");
        if (response?.quantity <= 0) {
            $(".add-to-cart-sticky-form .product-restock-request-section").show();
            productRestockRequestButton.removeAttr("disabled");
            $(".add-to-cart-sticky-form .product-add-and-buy-section").hide().removeClass("d-flex");
        } else {
            $(".add-to-cart-sticky-form .product-restock-request-section").hide();
            $(".add-to-cart-sticky-form .product-add-and-buy-section").show().addClass("d-flex");
        }
        if (response?.restock_request_status) {
            productRestockRequestButton.html(productRestockRequestButton.data("requested"));
            productRestockRequestButton.attr("disabled", true);
    ...
    if (response?.product_type?.toString() === "digital") {
        if (response?.quantity <= 0) {
            $(".add-to-cart-sticky-form .product-out-of-stock-section").show();
            $(".add-to-cart-sticky-form .product-add-and-buy-section").hide().removeClass("d-flex");
        } else {
            $(".add-to-cart-sticky-form .product-out-of-stock-section").hide();
            $(".add-to-cart-sticky-form .product-add-and-buy-section").show().addClass("d-flex");
        }
    }
}
document.addEventListener('change', function (e) {
    ...
    if (t.matches('.product_quantity__qty')) {
        const isSticky = t.closest('.add-to-cart-sticky-form') !== null;
        const newValue = t.value;

        if (isSticky) {
            $('.add-to-cart-details-form .product_quantity__qty').val(newValue);
        } else {
            $('.add-to-cart-sticky-form .product_quantity__qty').val(newValue);
        }
    }
});
...
document.addEventListener('DOMContentLoaded', function () {
    const coupons = document.querySelectorAll('.coupon-copy');
    coupons.forEach(span => {
        span.addEventListener('click', function () {
            const code = this.getAttribute('data-copy-coupun');
            navigator.clipboard.writeText(code).then(() => {
                toastr.success(document.getElementById('successfully-copied').getAttribute('data-text'));
            }).catch(err => {
...
$("#search-value").on("keyup", function () {
    ...
    if (!hasResult) {
        if ($("#no-result").length === 0) {
            $(".chat-list.chatting-scrollbar").append(
                '<div id="no-result" class="text-center text-muted py-3">No result found</div>'
            );
        }
    } else {
        $("#no-result").remove();
...
function initCartDropdown() {
    const $trigger = $('#mobile_app_bar .dropup a');

    window.openCart = function () {
        const $cartOverlay = $('#cart_overlay');
        const $cartDropdown = $('.cart-dropdown-mobile-view');

        $cartOverlay.show();
        $cartDropdown.addClass('show').stop(true, true).slideDown(300);
        $trigger.addClass('cart-open').removeClass('collapsed');
    };
    ...
    window.closeCart = function () {
        ...
        $cartOverlay.hide();
        $cartDropdown.removeClass('show').stop(true, true).slideUp(300);
        $trigger.removeClass('cart-open').addClass('collapsed');
    };

    $(document).off('click.cartTrigger').on('click.cartTrigger', '#mobile_app_bar .dropup a', function (e) {
        setTimeout(() => {
            e.preventDefault();
            if ($trigger.hasClass('cart-open')) {
                window.closeCart();
            } else {
                window.openCart();
            }
    ...
    $(document).off('click.closeCartBtn').on('click.closeCartBtn', '.close-cart-btn', function () {
        window.closeCart();
    });

    $(document).off('click.cartOverlay').on('click.cartOverlay', '#cart_overlay', function () {
        window.closeCart();
    });
}

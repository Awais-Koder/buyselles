            ...
            onChange: function (contents, $editable) {
                if ($(this).hasClass("product-description-default-language")) {
                    ...
                    if (textWithoutTagsAndEntities.length > maxLength) {
                        textWithoutTagsAndEntities = textWithoutTagsAndEntities.substring(
                            0,
                            maxLength
                        );
                    }
...
$("#colors-selector").on("change", function () {
    ...
    if ($("#product-color-switcher").prop("checked")) {
        // colorWiseImageFunctionality($("#colors-selector"));
        if (typeof colorWiseImageFunctionality === "function") {
            colorWiseImageFunctionality($("#colors-selector"));
        }
...
function uploadColorImage(thisData = null) {
    if (thisData) {
        let maxFileSizeLimit = Number(thisData?.dataset?.maxSize || ($('#imageUploadMaxSize').data('max-size') || 20));
        ...
        if (maxFileSizeLimitIAsByte && maxFileSizeLimitIAsByte < thisData.files[0]?.size) {
            toastMagic.error(thisData.files[0]?.name + ` exceeds ${maxFileSizeLimit}MB`);
        } else {
        ...
        if (maxFileSizeLimitIAsByte && maxFileSizeLimitIAsByte < thisData.files[0]?.size) {
            try {
                document.getElementById(thisData.dataset.imgpreview).setAttribute("src", '');
                document.getElementById(thisData.dataset.imgpreview).classList.add("d-none");
                const uploadWrapper = thisData.closest(".upload-file");
                setTimeout(() => {
                    if (uploadWrapper) {
...
function getUpdateDigitalVariationFunctionality() {
    ...
    $.ajax({
        type: "POST",
        url: $("#route-vendor-products-digital-variation-combination").data(
            "url"
        ),
        data: $("#product_form").serialize(),
        success: function (response) {
...
function deleteDigitalVariationFileFunctionality() {
    $(".digital-variation-file-delete-button").on("click", function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#route-vendor-products-digital-variation-file-delete").data(
                "url"
            ),
            data: {
...
function ProductVariationFileUploadFunctionality() {
    $('.variation-upload-item input[type="file"]').each(function () {
        $(this).on("change", function () {
            ...
            if (maxFileSizeLimitIAsByte && maxFileSizeLimitIAsByte < $(this)[0].files[0]?.size) {
                toastMagic.error($(this)[0].files[0]?.name + ` exceeds ${maxFileSizeLimit}MB`);
            } else {
                if (file) {
                    $(this)

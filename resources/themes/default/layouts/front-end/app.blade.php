...
<head>
    ...
    <link rel="stylesheet"
        href="{{ dynamicAsset(path: 'public/assets/backend/webfonts/uicons-regular-rounded.css') }}">
    ...
    <style>
        ...
        /* Hide the tooltip on hover for mailto and tel links */
        a[href^="mailto:"],
        a[href^="tel:"] {
            pointer-events: none;
    ...
    @if (env('APP_MODE') == 'demo')
        ...
        <script>
            ...
            })(window, document, 'script', 'dataLayer', 'GTM-TTLMKMHQ');
        </script>
...
<body class="toolbar-enabled {{ env('APP_MODE') == 'demo' ? 'demo' : '' }}" id="demo">
    @if (env('APP_MODE') == 'demo')
        ...
        <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TTLMKMHQ" height="0" width="0"
    ...
    <span class="d-none" id="text-validate-translate" data-required="{{ translate('this_field_is_required') }}"
    ...
    <script>
        (function() {
            var REFRESH_URL = '/captcha/math-refresh';

            function getKey(inp) {
                var c = inp.closest('[data-math-captcha-key]');
                return c ? c.getAttribute('data-math-captcha-key') : 'default_recaptcha_id_customer_auth';
            }

            function refreshCaptchaFor(inp) {
                fetch(REFRESH_URL + '?session_key=' + encodeURIComponent(getKey(inp)), {
                    ...
                    .then(function(d) {
                        var c = inp.closest('[data-math-captcha-key]') || inp.parentElement;
                        var q = c ? c.querySelector('[data-math-question]') || c.querySelector('span') : null;
            ...
            function refreshAll() {
                document.querySelectorAll('input[name="default_captcha_value"]').forEach(refreshCaptchaFor);
            }
            ...
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('input[name="default_captcha_value"]').forEach(function(inp) {
                    var parent = inp.parentElement;
                    if (!parent.querySelector('[data-refresh-captcha]')) {
                        var btn = document.createElement('button');
                        btn.type = 'button';
                        btn.className = 'btn btn-outline-secondary btn-sm';
                        btn.setAttribute('data-refresh-captcha', '');
                        btn.innerHTML = '&#8635;';
                        btn.addEventListener('click', function() {
                            refreshCaptchaFor(inp);
                        });
                        parent.insertBefore(btn, inp);
                    }
                });
            });
        })();

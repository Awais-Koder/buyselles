...
class EmailTemplateService
{
    ...
    public function getHiddenField(string $userType, string $templateName): ?array
    {

        $hiddenData = [
            EmailTemplateKey::REGISTRATION => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::REGISTRATION_VERIFICATION => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::REGISTRATION_FROM_POS => ['product_information', 'order_information', 'button_url', 'button_content_status', 'banner_image'],
            EmailTemplateKey::REGISTRATION_APPROVED => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::REGISTRATION_DENIED => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::ACCOUNT_ACTIVATION => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::ACCOUNT_SUSPENDED => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::ACCOUNT_UNBLOCK => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::ACCOUNT_BLOCK => ['product_information', 'order_information', 'button_content', 'banner_image'],
            EmailTemplateKey::DIGITAL_PRODUCT_DOWNLOAD => ['product_information', 'button_content', 'banner_image'],
    ...
    public function getAddData(string $userType, string $templateName, ?array $hideField, string $title, string $body): array
    {
        return [
            ...
            'copyright_text' => translate('copyright_') . date('Y') . ' ' . getWebConfig(name: 'company_name') . '. ' . translate('all_right_reserved') . '.',
    ...
    public function getUpdateData(object $request, $template): array
    {
        ...
        if ($request['image']) {
            $result['image'] = $this->update(dir: 'email-template/', oldImage: $template['image'], format: 'webp', image: $request->file('image'));
        }

        if ($request['icon']) {
            $result['image'] = $this->update(dir: 'email-template/', oldImage: $template['image'], format: 'webp', image: $request->file('icon'));
        }

        if ($request['logo']) {
            $result['logo'] = $this->update(dir: 'email-template/', oldImage: $template['logo'], format: 'webp', image: $request->file('logo'));
        }

        if ($request['banner_image']) {
            $result['banner_image'] = $this->update(dir: 'email-template/', oldImage: $template['banner_image'], format: 'webp', image: $request->file('banner_image'));
        }
